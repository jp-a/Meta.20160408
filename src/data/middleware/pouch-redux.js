...
function createPouchMiddleware( _paths ) {
    ...
    function listen( path ) {
        ...
        path.db.allDocs( { include_docs: true } ).then( function( results ){
            const nodes = results.rows.map( item => item.doc );
            nodes.forEach( node => {
                path.docs[ node._id ] = node;
            } );
            path.propagateInsert( nodes );
        } );
        ...
        path.db.changes( {
            ...
            include_docs: true,
            since: 'now'
        } ).on( 'change', change => onDbChange( path, change ) );
    }
